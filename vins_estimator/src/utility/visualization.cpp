...
ros::Publisher pub_linefeature, pub_linematch;
...
void registerPub(ros::NodeHandle &n)
{
    ...
    pub_linefeature = n.advertise<sensor_msgs::Image>("/vins_estimator/linefeature_img", 1000);
    pub_linematch = n.advertise<sensor_msgs::Image>("/vins_estimator/linematch_img", 1000);
...
void pubLineFeature(const Estimator &estimator, const std_msgs::Header &header)
{
    cv::Mat image1, image2;
    int idx = estimator.frame_count;
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        cv::cvtColor(estimator.images_show[idx], image1, cv::COLOR_GRAY2RGB);
        cv::undistort(image1, image2, estimator.cv_KMatrix, estimator.cv_dist);
        Eigen::Vector3d Tic(estimator.para_Ex_Pose[0][0], estimator.para_Ex_Pose[0][1], estimator.para_Ex_Pose[0][2]);
        Eigen::Matrix3d Ric = Quaterniond(estimator.para_Ex_Pose[0][6],
                                          estimator.para_Ex_Pose[0][3],
                                          estimator.para_Ex_Pose[0][4],
                                          estimator.para_Ex_Pose[0][5]).normalized().toRotationMatrix();

        Eigen::Vector3d Tbi (estimator.para_Pose[idx][0], estimator.para_Pose[idx][1], estimator.para_Pose[idx][2]);
        Eigen::Quaterniond Qbi(estimator.para_Pose[idx][6], estimator.para_Pose[idx][3], estimator.para_Pose[idx][4], estimator.para_Pose[idx][5]);
        // transfer Q into R rotation matrix form
        Eigen::Matrix3d Rbi = Qbi.normalized().toRotationMatrix();

        Eigen::Matrix3d ExRot = Ric.transpose() * Rbi.transpose();
        Eigen::Vector3d ExTrans = -ExRot * Tbi - Ric * Tic;


        sensor_msgs::ImagePtr msg = cv_bridge::CvImage(header, "bgr8", image2).toImageMsg();
        pub_linefeature.publish(msg);
    }
}

void pubLineMatch(const Estimator &estimator, const std_msgs::Header &header)
{
    cv::Mat image1, image2;
    int idx = estimator.frame_count;
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
